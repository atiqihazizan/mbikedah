...
export const DashboardViewModel = {
  build({ summary, viewer }) {
    ...
    if (app.completed > 0) {
      applicationCards.push({ key: 'completed', count: app.completed, label: 'Selesai', color: 'gray', navigateTo: '/permohonan/sejarah' })
    }

    const taskCards = []
    if (tasks.hodApproval)    taskCards.push({ key: 'hodApproval',    count: tasks.hodApproval,    label: 'Kelulusan Jabatan',    color: 'yellow', navigateTo: '/permohonan?status=PENDING_HOD' })
    if (tasks.ceoApproval)    taskCards.push({ key: 'ceoApproval',    count: tasks.ceoApproval,    label: 'Kelulusan CEO',        color: 'orange', navigateTo: '/permohonan?status=PENDING_CEO' })
    if (tasks.ceoFinal)       taskCards.push({ key: 'ceoFinal',       count: tasks.ceoFinal,       label: 'Kelulusan Muktamad',   color: 'purple', navigateTo: '/permohonan?status=PENDING_CEO_FINAL' })
    if (tasks.financeCheck)   taskCards.push({ key: 'financeCheck',   count: tasks.financeCheck,   label: 'Semakan Kewangan',     color: 'blue',   navigateTo: '/permohonan?status=PENDING_FINANCE_CHECK' })
    if (tasks.financeVerify)  taskCards.push({ key: 'financeVerify',  count: tasks.financeVerify,  label: 'Pengesahan Kewangan',  color: 'indigo', navigateTo: '/permohonan?status=PENDING_FINANCE_VERIFY' })
    if (tasks.financeApproval)taskCards.push({ key: 'financeApproval',count: tasks.financeApproval,label: 'Kelulusan Kewangan',   color: 'purple', navigateTo: '/permohonan?status=PENDING_FINANCE_APPROVAL' })
    if (tasks.payment)        taskCards.push({ key: 'payment',        count: tasks.payment,        label: 'Perlu Dibayar',        color: 'green',  navigateTo: '/permohonan?queue=payment' })
