...
import {
  ...
  Activity, ShieldCheck, KeyRound, Save, RefreshCw, Landmark,
} from 'lucide-react'
...
const TABS = [
  ...
  { id: 'bank',      label: 'Akaun Bank',         icon: Landmark },
...
const bankSchema = { code: '', name: '', bankName: '', accNo: '' }

function BankForm({ record, onClose, onSaved }) {
  const qc = useQueryClient()
  const isEdit = !!record?.id
  const [form, setForm] = useState(record ? { code: record.code, name: record.name, bankName: record.bankName, accNo: record.accNo } : { ...bankSchema })
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader><DialogTitle>{isEdit ? 'Kemaskini Akaun Bank' : 'Tambah Akaun Bank'}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3 mt-2">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Kod <span className="text-red-500">*</span></Label>
              <Input className="mt-1" placeholder="Cth: MBB01" value={form.code} onChange={e => set('code', e.target.value)} disabled={isEdit} />
            </div>
            <div>
          ...
          <div>
            ...
            <Input className="mt-1" placeholder="Nama pemilik akaun" value={form.name} onChange={e => set('name', e.target.value)} />
          </div>
          <div>
            <Label>No. Akaun <span className="text-red-500">*</span></Label>
            <Input className="mt-1" placeholder="Cth: 1234567890" value={form.accNo} onChange={e => set('accNo', e.target.value)} />
          </div>
          <DialogFooter className="mt-4">
            <Button type="button" variant="outline" onClick={onClose}>Batal</Button>
            <Button type="submit" disabled={mut.isPending}>
              {mut.isPending ? <Spinner className="w-4 h-4 mr-1.5" /> : null}
...
function TabBank() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{banks.length} akaun bank MBI</p>
