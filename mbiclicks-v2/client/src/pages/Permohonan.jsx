    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState title="Tiada permohonan" />
        ) : (
              ...
              {rows.map(row => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {vm.actionLabel !== 'Lihat' ? (
                        <button onClick={() => navigate(vm.actionPath)}
                          className={`p-1.5 text-${vm.actionColor}-600 hover:text-${vm.actionColor}-700 rounded hover:bg-${vm.actionColor}-50 font-medium text-xs`}>
                          {vm.actionLabel}
                        </button>
                      ) : (
                        <button onClick={() => navigate(vm.actionPath)}
