...
export default function FinanceKelulusan() {
  ...
  // Restrict access — Finance HOD only
  const roleSlug = user?.role?.slug
  if (!['finance_hod', 'admin'].includes(roleSlug)) {
    return (
  ...
  if (error?.response?.status === 403 || error?.response?.status === 404) {
    return <ApprovalPageBase billing={null} isLoading={false} title="Kelulusan Kewangan" />
  }
