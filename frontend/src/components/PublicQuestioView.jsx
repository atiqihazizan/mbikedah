function PublicQuestioView({ question, index, answerChanged }) {
  let selectedOptions = [];
        ...
        <div className="mt-3">
              ...
              {question.data.options.map((option, ind) => (
                <div key={option.uuid} className="flex items-center">
                  <input
                    ...
                    onChange={(ev) => answerChanged(ev.target.value)}
                    type="radio"
                    className="focus:fing-indigo-500 h-4 w-4 text-indigo-600 border-gray-300"
                  />
                  <label
                    for={option.uuid}
          ...
          {question.type === "text" && (
            <div>
              <input
                type="text"
                onChange={(ev) => answerChanged9ev.target.value}
...
export default PublicQuestioView;
