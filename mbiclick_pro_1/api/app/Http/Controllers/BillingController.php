...
class BillingController extends Controller
{
  ...
  public function store(BillingRequest $request)
  {
    try {
      ...
      $initialStatus = $validatedData['status_id'] ?? BillingStatus::DRAFT;
      
      // If applicant is HOD, skip to FINANCE_REVIEW
      if ($request->user()->hasAbility(UserAbilities::HOD) && $initialStatus === BillingStatus::HOD_APPROVAL) {
        $initialStatus = BillingStatus::FINANCE_REVIEW;
      }
      ...
      // Handle status progression based on final status
      if ($initialStatus === BillingStatus::HOD_APPROVAL) {
        $billing->history()->create([
      ...
      } elseif ($initialStatus === BillingStatus::FINANCE_REVIEW) {
        // HOD skipped - direct transition from DRAFT to FINANCE_REVIEW
        $billing->history()->create([
          'old_status' => BillingStatus::DRAFT,
          'new_status' => BillingStatus::FINANCE_REVIEW,
          'created_by' => $request->user()->id,
          'remarks' => $request->remarks ?? 'Hantar ke Kewangan (Skip HOD)',
          'status_name' => BillingStatus::getNameStatus(BillingStatus::FINANCE_REVIEW)
        ]);
      }
  ...
  public function update(BillingRequest $request, Billing $billing)
  {
    try {
      ...
      if ($validatedData['status_id'] === BillingStatus::HOD_APPROVAL) {
        // If user is HOD, skip to FINANCE_REVIEW directly
        if ($request->user()->hasAbility(UserAbilities::HOD)) {
          $billing->updateStatus(BillingStatus::FINANCE_REVIEW, Auth::id(), $request->remarks ?? 'Hantar ke Kewangan (Skip HOD)');
        } else {
