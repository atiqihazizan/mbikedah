...
export default function BillingFormDialog({ show, onClose, onSaved, billingId = null, mode = "create" }) {
  ...
  const submitToHOD = async () => {
    ...
    // If user is HOD, submit directly to FINANCE_REVIEW (3), otherwise HOD_APPROVAL (2)
    const targetStatus = currentUser?.abilities?.includes(3) ? 3 : 2;
    
    const result = await saveFormMutation(petition, billingId, targetStatus);
    if (result.success) {
                  ...
                  <>
                    ...
                    {canEdit && (
                      <>
                        ...
                        <TButton color="primary" onClick={submitToHOD} isDisable={!checkValid()} onChecking={loading}>
                          {currentUser?.abilities?.includes(3) ? 'Hantar ke Kewangan' : 'Hantar ke Ketua Jabatan'}
                        </TButton>
                      </>
                    )}
                  </>
