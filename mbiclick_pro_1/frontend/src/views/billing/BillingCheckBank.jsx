        ...
        <div className="mt-1 flex gap-6">
          {paymentMethods.map((method) => (
            <label key={method.value} className="flex items-center space-x-2 cursor-pointer">
              <input type="radio" name="payment_method" value={method.value} checked={billing?.payment_method === method.value} onChange={(e) => setBilling({...billing, payment_method: e.target.value})} disabled={processing} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"/>
    ...
    <tr>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 align-top">
        <div className="space-y-2">
          <select value={isExistingTransaction ? transaction.bank_id : preTransactions.bank_id} onChange={handleChangeTransaction} required className="input w-full">
