...
@section('toolbar')
    <div class="d-flex flex-column align-items-start me-3 gap-2">
        <h1 class="d-flex text-dark fw-bolder m-0 fs-3">Lojing Hotel</h1>
        <ul class="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7" id="kt_toolbar_bread">
            ...
            <li class="breadcrumb-item text-gray-500">Lojing</li>
        </ul>
    </div>
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_booking">
                ...
                <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                    <th class="d-none"></th>
                    <th class="text-center w-60px">Pada</th>
                    <th class="text-center w-60px">Hingga</th>
                ...
                @foreach($data as $d)
                    <tr>
                        ...
                        <td class="text-center">{{\Carbon\Carbon::parse($d->dtstart)->format('d M y')}}</td>
                        <td class="text-center">{{\Carbon\Carbon::parse($d->dtuntil)->format('d M y')}}</td>
                        <td class="">{{$d->ref}}</td>
                        <td class="">{{$d->descrip}}</td>
                        <td class="">{{Str::title($d->staff->fullname)}}</td>
                        <td class="">{{Str::title($d->depart->name)}}</td>
                        <td class="">{{$d->remark}}</td>
                        {{--<td class="text-end">
